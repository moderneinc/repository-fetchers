...
while getopts ":g:h:a" opt; do
    case ${opt} in
        ...
        a )
            ...
            ;;
        \? )
            echo "Usage: gitlab.sh [-g <group>] [-h <gitlab_domain>] [-a]"
            echo ""
            echo "Options:"
            ...
            echo "  -a                Include all repos. Only applies without -g. By default the"
            echo "                    query is limited to projects you are a member of; this"
            echo "                    returns every project visible to your token instead, which"
            ...
            echo ""
            echo "  # Fetch every project visible to your token"
            echo "  AUTH_TOKEN=your_token ./gitlab.sh -a"
            echo ""
...
if [[ -z $GROUP ]]; then
    # Limit to projects the user is a member of unless -a was passed
    if [[ $INCLUDE_ALL_REPOS == true ]]; then
        membership=""
    else
        membership="membership=true&"
    fi
    base_request_url="$GITLAB_DOMAIN/api/v4/projects?${membership}simple=true&archived=false"
else
